...
#define TIZEN_DEFAULT_ICON ICONDIR"/default_icon.png"
...
static bool _notification_accept(notification_h noti)
{
	...
	return applist & NOTIFICATION_DISPLAY_APP_TICKER;
}
...
{
	retv_if(!path, INDICATOR_ANIMATED_ICON_NONE);
...
static char *_pkginfo_get_icon(const char *pkgid)
{
	...
	retvm_if(!pkgid, NULL, invalid parameter);
...
{
	retvm_if(!ticker.win || !ticker.layout, FAIL,
	...
	retvm_if(!textblock, FAIL, "_ticker_textblock_create failed");
...
{
	retvm_if(_ticker_textblock_update(ticker_info), FAIL,
			"_ticker_textblock_update failed");
	retvm_if(_ticker_icon_update(ticker_info), FAIL,
...
static Evas_Object *_ticker_icon_create(ticker_info_s *ticker_info)
{
	...
	if (!icon) {
		...
		if (!icon_path
			...
			app_icon_path = _pkginfo_get_icon(ticker.current->pkgname);
			if (app_icon_path) {
				elm_image_file_set(icon, app_icon_path, NULL);
				elm_image_resizable_set(icon, EINA_TRUE, EINA_TRUE);
				free(app_icon_path);
			} else {
				ret = elm_image_file_set(icon, TIZEN_DEFAULT_ICON, NULL);
				retvm_if(!ret, NULL, "elm_image_file_set failed");
...
{
	ticker.ad = (struct appdata *)data;

	int ret = notification_register_detailed_changed_cb(_ticker_noti_detailed_changed_cb, data);
	if (ret != NOTIFICATION_ERROR_NONE) {
