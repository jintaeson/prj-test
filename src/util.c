...
char *util_get_real_path(char *special_path)
{
	...
	for (i = 0; i < ARRAY_SIZE(reserved_paths); ++i) {
		...
		if (!strcmp(special_path, reserved_paths[i].path)) {
			...
			size = strlen(util_get_icon_dir()) + strlen(reserved_paths[i].icon) + 2; /* +2 is for '/' and NULL*/
			real_path = calloc(size, sizeof(char));

			if (!real_path)
				return NULL;

			snprintf(real_path, size, "%s/%s", util_get_icon_dir(), reserved_paths[i].icon);
