...
static void indicator_battery_check_charging(void *data)
{
	...
	_D("Charging status:%s", (status) ? "TRUE" : "FALSE");
...
static int register_battery_module(void *data)
{
	...
	if (ret != DEVICE_ERROR_NONE) {
		_E("device_add_callback failed[%d]:%s", ret, get_error_message(ret));
		r = FAIL;
	}
...
static int unregister_battery_module(void)
{
	...
	ret |= vconf_ignore_key_changed(VCONFKEY_SETAPPL_BATTERY_PERCENTAGE_BOOL, indicator_battery_batt_percentage_cb);

	if (ret != OK) _E("unregister_battery_module failed");
